...
function SkeletonCard({ width }: { width: number }) {
  ...
  return <Animated.View style={{ width, height: width, borderRadius: 16, backgroundColor: COLORS.surface, opacity, margin: 4 }} />;
}
...
function GhostCard({ width }: { width: number }) {
  return (
    <AnimatedPressable
      onPress={() => {
        console.log('[Home iOS] Ghost card pressed — opening add person');
...
export default function HomeScreen() {
  ...
  const handleLongPress = (person: Person) => {
    console.log('[Home iOS] Long press on person:', person.id, person.name);
    Alert.alert(person.name, 'What would you like to do?', [
      { text: 'Cancel', style: 'cancel' },
      {
        ...
        style: 'destructive',
        onPress: () => {
          console.log('[Home iOS] Moving to bench:', person.id);
  ...
  const renderCard = ({ item, index }: { item: Person; index: number }) => {
    ...
    return (
      <AnimatedCard index={index}>
        <AnimatedPressable
          onPress={() => {
            console.log('[Home iOS] Navigating to person:', item.id);
        ...
        options={{
          ...
          headerRight: () => (
            <View style={{ flexDirection: 'row', gap: 8, marginRight: 4 }}>
              <AnimatedPressable
                onPress={() => {
                  console.log('[Home iOS] Analytics pressed');
              ...
              <AnimatedPressable
                onPress={() => console.log('[Home iOS] Menu pressed')}
                style={{
                  width: 36,
                  height: 36,
                  borderRadius: 18,
      ...
      {loading ? (
        <View style={{ flex: 1, padding: 12 }}>
          <View style={{ flexDirection: 'row', flexWrap: 'wrap' }}>
            {[0, 1, 2, 3].map((i) => <SkeletonCard key={i} width={cardWidth} />)}
          </View>
        </View>
      ) : error ? (
