...
export function registerPersonsRoutes(app: App) {
              ...
              persons: {
                ...
                items: {
                  ...
                  properties: {
                    ...
                    overall_chemistry: { type: ['integer', 'null'] },
                    communication: { type: ['integer', 'null'] },
                    consistency: { type: ['integer', 'null'] },
                    emotional_availability: { type: ['integer', 'null'] },
                    date_planning: { type: ['integer', 'null'] },
                    alignment: { type: ['integer', 'null'] },
                    connection_type: { type: ['string', 'null'] },
                    connection_type_custom: { type: ['string', 'null'] },
  ...
  app.fastify.get(
    ...
    async (request: FastifyRequest<{ Params: { id: string } }>, reply: FastifyReply) => {
      ...
      console.log('[GET /persons/:id] returning fields:', Object.keys(person).join(', '));
      app.logger.info({ userId: session.user.id, personId: id }, 'Person retrieved');
      return { person: toSnakePerson(person) };
    }
  );
